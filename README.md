...
👉 **[`lll_wiki/index.md`](lll_wiki/index.md)** 
L'indice principale contiene il catalogo di tutte le pagine generate, raggruppate per macro-categorie.

### 📂 Struttura delle directory

Tutto il nucleo del sapere è racchiuso nella cartella `lll_wiki/`, strutturata secondo un pattern architetturale semantico:

- `lll_wiki/wiki/sources/` 📚: Note di partenza, trascrizioni di lezioni (es. *Anthropic Academy*), documentazione sulle API, tool e RAG.
- `lll_wiki/wiki/concepts/` 💡: Concetti distillati e spiegati in dettaglio (es. *Prompt Evaluation*, *Prompt Caching*, *RAG Hybrid Search*, *MCP Protocol*, *Agent Systems*).
- `lll_wiki/wiki/entities/` 🏢: Entità chiave menzionate nella base di conoscenza (es. *Anthropic*, *Claude*, *VoyageAI*).
- `lll_wiki/wiki/synthesis/` 🧪: Articoli di sintesi che uniscono concetti multipli in guide strutturate (es. *Definizione di System Prompt*).
- `lll_wiki/raw/` 📥: Materiale grezzo o log di importazione in attesa di essere processato.

### 📜 File speciali
- `lll_wiki/CLAUDE.md`: Istruzioni di sistema (System Prompt) per l'agente AI che gestisce e aggiorna la Wiki.
- `lll_wiki/log.md`: Registro delle attività e degli aggiornamenti (ingest) effettuati sulla Wiki.
